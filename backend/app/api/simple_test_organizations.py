...
from app.models.models import User, Organization, Membership, RoleEnum
...
def create_test_user(db, username, email, is_superuser=False):
    ...
    user = User(
        username=username,
        email=email,
        hashed_password=AuthHandler.hash_password("test123"),
        full_name="Test User",
        is_active=True,
...
def test_client():
    ...
    # Create JWT token
    token = AuthHandler.create_access_token(data={"sub": user.username})
...
class TestOrganizationsRoutes:
    ...
    def test_update_organization_as_non_owner(self, test_client):
        ...
        member_token = AuthHandler.create_access_token(data={"sub": member_user.username})
    ...
    def test_list_members_as_member(self, test_client):
        ...
        # Create token for member user
        member_token = AuthHandler.create_access_token(data={"sub": another_user.username})
    ...
    def test_remove_member_as_non_owner(self, test_client):
        ...
        admin_token = AuthHandler.create_access_token(data={"sub": admin_user.username})
